...
export function registerTagRoutes(router: AppRouter) {
  router.get('/api/tags', async (_request, env) => {
    const service = createService(env);
    const tags = await service.listTags();
    return Response.json(tags);
  });

  router.post('/api/tags', async (request, env) => {
    const service = createService(env);
  ...
  router.put('/api/tags/:id', async (request, env) => {
    const service = createService(env);
    const body = await request.json();
  ...
  router.delete('/api/tags/:id', async (request, env) => {
    const service = createService(env);
    await service.deleteTag(request.params.id);
    return new Response(null, { status: 204 });
  });

  router.get('/api/surveys/:id/tags', async (request, env) => {
    const service = createService(env);
    const tags = await service.getTagsForSurvey(request.params.id);
    return Response.json(tags);
  });

  router.put('/api/surveys/:id/tags', async (request, env) => {
    const service = createService(env);
