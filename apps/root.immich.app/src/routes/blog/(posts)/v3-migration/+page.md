...
:::warning
This migration is still being written and are not yet complete. Some sections may be missing, incomplete, or subject to change before the final version is published.

:::

Everyone has been busy breaking Immich :sweat_smile:
...
:::info
Use the `machineLearning.availabilityChecks.timeout` system config instead (available in the admin settings page)
...
#### `PUT /assets/:id/original` `replaceAssest`

This endpoint has been removed. The API Key permission for this endpoint, `asset.replace`, has also been removed.

:::info
Use `PUT /assets/:id/clone` (`copyAsset`) instead for similar functionality.

:::

#### `GET /assets/random` / `getRandom`
...
#### `POST /assets/exists` / `checkExistingAssets`

This endpoint has been removed.

#### `GET /assets/device/:deviceId` / `getAllUserAssetsByDeviceId`
